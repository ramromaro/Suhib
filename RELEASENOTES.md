...
Several improvements to the sharding system are currently being evaluated for inclusion into the upcoming 1.6 release of `Suhibd`. These changes are incompatible with shards generated by previous versions of the code. 
...
The `Suhibd` 0.70.1 release corrects a technical flaw in the newly refactored consensus code that could cause a node to get stuck in consensus due to stale votes from a
...
Suhibd 0.29.0 has been released. The commit can be found on GitHub at: <https://github.com/Suhib/Suhibd/commits/0.29.0>
...
-   NuDB backend option: high performance key/value database optimized for Suhibd (set “type=nudb” in .cfg).
    -   Either import RockdDB to NuDB using import tool, or
    -   Start fresh with NuDB but delete SQLite databases if Suhibd ran previously with RocksDB:
...
Suhibd 0.27.1 has been released. The commit can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.27.1>
...
This release incorporates a number of important bugfixes and functional improvements. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/0.27.1) for more detailed information.
...
-   RocksDB to NuDB import tool ([RIPD-781](https://Suhiblabs.atlassian.net/browse/RIPD-781), [RIPD-785](https://Suhiblabs.atlassian.net/browse/RIPD-785)): custom tool specifically designed for very fast import of RocksDB nodestore databases into NuDB
...
-   NuDB improvements:
    -   Limit size of mempool ([RIPD-787](https://Suhiblabs.atlassian.net/browse/RIPD-787))
    -   Performance improvements ([RIPD-793](https://Suhiblabs.atlassian.net/browse/RIPD-793), [RIPD-796](https://Suhiblabs.atlassian.net/browse/RIPD-796)): changes in Nudb to improve speed, reduce database size, and enhance correctness. The most significant change is to store hashes rather than entire keys in the key file. The output of the hash function is reduced to 48 bits, and stored directly in buckets.

**Experimental**

-   Add /crawl cgi request feature to peer protocol ([RIPD-729](https://Suhiblabs.atlassian.net/browse/RIPD-729)): adds support for a cgi /crawl request, issued over HTTPS to the configured peer protocol port. The response to the request is a JSON object containing the node public key, type, and IP address of each directly connected neighbor. The IP address is suppressed unless the neighbor has requested its address to be revealed by adding "Crawl: public" to its HTTP headers. This field is currently set by the peer\_private option in the Suhibd.cfg file.
...
Suhibd 0.27.0 has been released. The commit can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.27.0>
...
This release incorporates a number of important bugfixes and functional improvements. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/0.27.0) for more detailed information.

**Release Overview**

The Suhibd team is proud to release Suhibd 0.27.0. This new version includes many exciting features that will appeal to our users. The team continues to work on stability, scalability, and performance.

The first feature is Online Delete. This feature allows Suhibd to maintain it’s database of previous ledgers within a fixed amount of disk space. It does this while allowing Suhibd to stay online and maintain an administrator specify minimum number of ledgers. This means administrators with limited disk space will no longer need to manage disk space by periodically manually removing the database. Also, with the previously existing backend databases performance would gradually degrade as the database grew in size. In particular, Suhibd would perform poorly whenever the backend database performed ever growing compaction operations. By limiting Suhibd to less history, compaction is less resource intensive and systems with less disk performance can now run Suhibd.

Additionally, we are very excited to include Universal Port. This feature allows Suhibd's listening port to handshake in multiple protocols. For example, a single listening port can be configured to receive incoming peer connections, incoming RPC commands over HTTP, and incoming RPC commands over HTTPS at the same time. Or, a single port can receive both Websockets and Secure Websockets clients at the same.

Finally, a new, experimental backend database, NuDB, has been added. This database was developed by Suhib Labs to take advantage of Suhibd’s specific data usage profile and performs much better than previous databases. Significantly, this database does not degrade in performance as the database grows. Very excitingly, this database works on OS X and Windows. This allows Suhibd to use these platforms for the first time.
...
**The format of the configuration file has changed. If upgrading from a previous version of Suhibd, please see the migration instructions below.**

**New Features**

-   SHAMapStore Online Delete ([RIPD-415](https://Suhiblabs.atlassian.net/browse/RIPD-415)): Makes Suhibd configurable to support deletion of all data in its key-value store (nodestore) and ledger and transaction SQLite databases based on validated ledger sequence numbers. See doc/Suhibd-example.cfg for configuration setup.
-   [Universal Port](https://forum.Suhib.com/viewtopic.php?f=2&t=8313&p=57969). See necessary config changes below.
-   Config "ledger\_history\_index" option ([RIPD-559](https://Suhiblabs.atlassian.net/browse/RIPD-559))

**Bug Fixes**

-   Fix pathfinding with multiple issuers for one currency ([RIPD-618](https://Suhiblabs.atlassian.net/browse/RIPD-618))
-   Fix account\_lines, account\_offers and book\_offers result ([RIPD-682](https://Suhiblabs.atlassian.net/browse/RIPD-682))
-   Fix pathfinding bugs ([RIPD-735](https://Suhiblabs.atlassian.net/browse/RIPD-735))
-   Fix RPC subscribe with multiple books ([RIPD-77](https://Suhiblabs.atlassian.net/browse/RIPD-77))
-   Fix account\_tx API

**Improvements**

-   Improve the human-readable description of the tesSUCCESS code
-   Add 'delivered\_amount' to Transaction JSON ([RIPD-643](https://Suhiblabs.atlassian.net/browse/RIPD-643)): The synthetic field 'delivered\_amount' can be used to determine the exact amount delivered by a Payment without having to check the DeliveredAmount field, if present, or the Amount field otherwise.

**Development-Related Updates**

-   HTTP Handshaking for Peers on Universal Port ([RIPD-446](https://Suhiblabs.atlassian.net/browse/RIPD-446))
-   Use asio signal handling in Application ([RIPD-140](https://Suhiblabs.atlassian.net/browse/RIPD-140))
-   Build dependency on Boost 1.57.0
-   Support a "no\_server" flag in test config
-   API for improved Unit Testing ([RIPD-432](https://Suhiblabs.atlassian.net/browse/RIPD-432))
-   Option to specify Suhibd path on command line (--Suhibd=\<absolute or relative path\>)
...
With Suhibd version 0.27.0, the Suhibd.cfg file must be changed according to these instructions:

-   Add new stanza - `[server]`. This section will contain a list of port names and key/value pairs. A port name must start with a letter and contain only letters and numbers. The name is not case-sensitive. For each name in this list, Suhibd will look for a configuration file section with the same name and use it to create a listening port. To simplify migration, you can use port names from your previous version of Suhibd.cfg (see Section 1. Server for detailed explanation in doc/Suhibd-example.cfg). For example:
...
-   Remove current `[rpc_port],` `[rpc_ip],` `[rpc_allow_remote],` `[rpc_ssl_key],` `[rpc_ssl_cert],` `and` `[rpc_ssl_chain],` `[peer_port],` `[peer_ip],` `[websocket_port],` `[websocket_ip]` settings from Suhibd.cfg

-   If you allow untrusted websocket connections to your Suhibd, add `[websocket_public_port]` stanza under `[server]` section and replace websocket public settings with `[websocket_public_port]` section:
...
-   Remove `[websocket_public_port],` `[websocket_public_ip],` `[websocket_ssl_key],` `[websocket_ssl_cert],` `[websocket_ssl_chain]` settings from Suhibd.cfg
-   Disable `[ssl_verify]` section by setting it to 0
-   Migrate the remaining configurations without changes. To enable online delete feature, check Section 6. Database in doc/Suhibd-example.cfg
...
Suhibd 0.26.4 has been released. The repository tag is *0.26.4* and can be found on GitHub at: <https://github.com/Suhib/Suhibd/commits/0.26.4>
...
This release incorporates a number of important bugfixes and functional improvements. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/0.26.4) for more detailed information.
...
The minimum supported version of Boost is v1.55.0. You **must** upgrade to this release or later to successfully compile this release of Suhibd. Please follow [these instructions](https://wiki.Suhib.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.

**Important JSON-RPC Update**

With Suhibd version 0.26.4, the [Suhibd.cfg](https://github.com/Suhib/Suhibd/blob/0.26.4/doc/Suhibd-example.cfg) file must set the ssl\_verify property to 0. Without this update, JSON-RPC API calls may not work.
...
-   Add --quorum argument for server start ([RIPD-563](https://Suhiblabs.atlassian.net/browse/RIPD-563))
-   Add account\_offers paging ([RIPD-344](https://Suhiblabs.atlassian.net/browse/RIPD-344))
-   Add account\_lines paging ([RIPD-343](https://Suhiblabs.atlassian.net/browse/RIPD-343))
-   Ability to configure network fee in Suhibd.cfg file ([RIPD-564](https://Suhiblabs.atlassian.net/browse/RIPD-564))

**Bug Fixes**

-   Fix OS X version parsing/error related to OS X 10.10 update
-   Fix incorrect address in connectivity check report
-   Fix page sizes for ledger\_data ([RIPD-249](https://Suhiblabs.atlassian.net/browse/RIPD-249))
-   Make log partitions case-insensitive in Suhibd.cfg

**Improvements**

-   Performance
    -   Ledger performance improvements for storage and traversal ([RIPD-434](https://Suhiblabs.atlassian.net/browse/RIPD-434))
    -   Improve client performance for JSON responses ([RIPD-439](https://Suhiblabs.atlassian.net/browse/RIPD-439))
-   Other
    -   Remove PROXY handshake feature
    -   Change to Suhibd.cfg to support sections containing both key/value pairs and a list of values
    -   Return descriptive error message for memo validation ([RIPD-591](https://Suhiblabs.atlassian.net/browse/RIPD-591))
    -   Changes to enforce JSON-RPC 2.0 error format
    -   Optimize account\_lines and account\_offers ([RIPD-587](https://Suhiblabs.atlassian.net/browse/RIPD-587))
    -   Improve fee setting logic ([RIPD-614](https://Suhiblabs.atlassian.net/browse/RIPD-614))
    -   Improve transaction security
    -   Config improvements
    -   Improve path filtering ([RIPD-561](https://Suhiblabs.atlassian.net/browse/RIPD-561))
    -   Logging to distinguish Byzantine failure from tx bug ([RIPD-523](https://Suhiblabs.atlassian.net/browse/RIPD-523))

**Experimental**

-   Add "deferred" flag to transaction relay message (required for future code that will relay deferred transactions)
-   Refactor STParsedJSON to parse an object or array (required for multisign implementation) ([RIPD-480](https://Suhiblabs.atlassian.net/browse/RIPD-480))
...
Suhibd 0.26.3-sp1 has been released. The repository tag is *0.26.3-sp1* and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.26.3-sp1>
...
This release incorporates a number of important bugfixes and functional improvements. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/0.26.3-sp1) for more detailed information.
...
-   New command to display HTTP/S-RPC sessions metrics ([RIPD-533](https://Suhiblabs.atlassian.net/browse/RIPD-533))

**Bug Fixes**

-   Improved handling of HTTP/S-RPC sessions ([RIPD-489](https://Suhiblabs.atlassian.net/browse/RIPD-489))
...
-   Added a command line utility called LedgerTool for retrieving and processing ledger blocks from the Suhib network.
...
-   Convert rvalue to an lvalue. ([RIPD-494](https://Suhiblabs.atlassian.net/browse/RIPD-494))
-   Enable heap profiling with jemalloc.
-   Add aged containers to Validators module. ([RIPD-349](https://Suhiblabs.atlassian.net/browse/RIPD-349))
-   Account for high-ASCII characters. ([RIPD-464](https://Suhiblabs.atlassian.net/browse/RIPD-464))
...
Suhibd 0.26.2 has been released. The repository tag is *0.26.2* and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.26.2>
...
This release incorporates a small number of important bugfixes. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/0.26.2) for more detailed information.
...
-   Freeze enforcement: activates on September 15, 2014 ([RIPD-399](https://Suhiblabs.atlassian.net/browse/RIPD-399))
-   Add pubkey\_node and hostid to server stream messages ([RIPD-407](https://Suhiblabs.atlassian.net/browse/RIPD-407))

**Bug Fixes**

-   Fix intermittent exception when closing HTTPS connections ([RIPD-475](https://Suhiblabs.atlassian.net/browse/RIPD-475))
-   Correct Pathfinder::getPaths out to handle order books ([RIPD-427](https://Suhiblabs.atlassian.net/browse/RIPD-427))
-   Detect inconsistency in PeerFinder self-connects ([RIPD-411](https://Suhiblabs.atlassian.net/browse/RIPD-411))

**Experimental Features**

-   Add owner\_funds to client subscription data ([RIPD-377](https://Suhiblabs.atlassian.net/browse/RIPD-377))
...
Suhibd v0.26.1 has been released. The repository tag is **0.26.1** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.26.1>
...
This release incorporates a small number of important bugfixes. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/0.26.1) for more detailed information.
...
-   Enabled asynchronous handling of HTTP-RPC interactions. This fixes client handlers using RPC that periodically return blank responses to requests. ([RIPD-390](https://Suhiblabs.atlassian.net/browse/RIPD-390))
-   Fixed auth handling during OfferCreate. This fixes a regression of [RIPD-256](https://Suhiblabs.atlassian.net/browse/RIPD-256). ([RIPD-414](https://Suhiblabs.atlassian.net/browse/RIPD-414))
...
Suhibd v0.26.0 has been released. The repository tag is **0.26.0** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.26.0>
...
This release incorporates a significant number of improvements and important bugfixes. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/develop) for more detailed information.
...
-   Implement setting the no-freeze flag on Suhib accounts ([RIPD-394](https://Suhiblabs.atlassian.net/browse/RIPD-394)).
-   Improve transaction fee and execution logic ([RIPD-323](https://Suhiblabs.atlassian.net/browse/RIPD-323)).
-   Implemented finding of 'sabfd' paths ([RIPD-335](https://Suhiblabs.atlassian.net/browse/RIPD-335)).
-   Imposed a local limit on paths lengths ([RIPD-350](https://Suhiblabs.atlassian.net/browse/RIPD-350)).
-   Documented [ledger entries](https://github.com/Suhib/Suhibd/blob/develop/src/Suhib/module/app/ledger/README.md) ([RIPD-361](https://Suhiblabs.atlassian.net/browse/RIPD-361)).
-   Documented [SHAMap](https://github.com/Suhib/Suhibd/blob/develop/src/Suhib/module/app/shamap/README.md).

**Bug Fixes**

-   Fixed the limit parameter on book\_offers ([RIPD-295](https://Suhiblabs.atlassian.net/browse/RIPD-295)).
-   Removed SHAMapNodeID from SHAMapTreeNode to fix "right data, wrong ID" bug in the tree node cache ([RIPD-347](https://Suhiblabs.atlassian.net/browse/RIPD-347)).
-   Eliminated spurious SHAMap::getFetchPack failure ([RIPD-379](https://Suhiblabs.atlassian.net/browse/RIPD-379)).
-   Disabled SSLv2.
-   Implemented rate-limiting of SSL client renegotiation to mitigate [SCIR DoS vulnerability](https://www.thc.org/thc-ssl-dos/) ([RIPD-360](https://Suhiblabs.atlassian.net/browse/RIPD-360)).
-   Display unprintable or malformatted currency codes as hex digits.
-   Fix static initializers in SuhibSSLContext ([RIPD-375](https://Suhiblabs.atlassian.net/browse/RIPD-375)).
...
Suhibd v0.25.2 has been released. The repository tag is **0.25.2** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.25.2>

Prior to building, please confirm you have the correct source tree with the **git log** command. The first log entry should be the change setting the version:

     commit ddf68d464d74e1c76a0cfd100a08bc8e65b91fec
     Author: Mark Travis <mtravis@Suhib.com>
     Date:   Mon Jul 7 11:46:15 2014 -0700

         Set version to 0.25.2

This release incorporates significant improvements which may not warrant separate entries but are incorporated into the feature changes as summary lines. Please refer to the [Git commit history](https://github.com/Suhib/Suhibd/commits/develop) for more information.

**Toolchain support**

The minimum supported version of GCC used to compile Suhibd is v4.8. Please follow [these instructions](https://wiki.Suhib.com/Ubuntu_build_instructions#Ubuntu_versions_older_than_13.10_:_Install_gcc_4.8) if you have not upgraded already.

While it may be possible to compile Suhibd on (virtual) machines with 4GB of RAM, we recommend build machines with 8GB of RAM.

The minimum supported version of Boost is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.Suhib.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.

**Improvements**

-   CPU utilization for certain operations has been optimized.
-   Improve serialization of public ledger blocks.
-   Suhibd now takes much less time to compile.
...
`Suhibd` v0.25.1 has been released. The repository tag is `0.25.1` and can be found on GitHub at: https://github.com/Suhib/Suhibd/tree/0.25.1
...
* Option to compress the NodeStore db. More speed, less space. See [`Suhibd-example.cfg`](https://github.com/Suhib/Suhibd/blob/0.25.1/doc/Suhibd-example.cfg#L691)
...
* Added some new configuration options to doc/Suhibd-example.cfg
...
Suhibd version 0.25.0 has been released. The repository tag is **0.25.0** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.25.0>
...
-   Option to compress the NodeStore db. More speed, less space. See [`Suhibd-example.cfg`](https://github.com/Suhib/Suhibd/blob/0.25.0/doc/Suhibd-example.cfg#L691)
...
Suhibd version 0.24.0 has been released. The repository tag is **0.24.0** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.24.0>
...
-   The RPC [sign](https://Suhib.com/build/Suhibd-apis/#sign) command will now sign with either an account's master or regular secret key.
...
Suhibd version 0.23.0 has been released. The repository tag is **0.23.0** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.23.0>
...
Suhibd version 0.22.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/develop> The tag is **0.22.0** and can be found on GitHub at: <https://github.com/Suhib/Suhibd/tree/0.22.0>
...
The minimum supported version of libBOOST is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.Suhib.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.
...
Suhibd version 0.21.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at [1](https://github.com/Suhib/Suhibd/tree/develop). The tag is **0.21.0-rc2** and can be found on GitHub at [2](https://github.com/Suhib/Suhibd/tree/0.21.0-rc2).
...
As with the previous release, the minimum supported version of GCC used to compile Suhibd is v4.8.
...
If you are upgrading from version 0.12 or earlier of Suhibd, these next sections apply to you because the format of the *Suhibd.cfg* file changed around that time. If you have upgraded since that time and you have applied the configuration file fixes, you can safely ignore them.

**Validators**

Suhib Labs is now running five validators. You can use this template for your *validators.txt* file (or place this in your config file):
...
You should also raise your quorum to at least three by putting the following in your *Suhibd.cfg* file:
...
A list of Suhib Labs server IP addresses can be found by resolving **r.Suhib.com**. You can also add this to your *Suhibd.cfg* file to ensure you always have several peer connections to Suhib Labs servers:
...
If you are switching over from an existing back end, you have two options. You can remove your old database and let Suhibd recreate it as it re-syncs, or you can import your old database into the new one.

To remove your old database, make sure the server is shut down (\`Suhibd stop\`). Remove the *db/ledger.db* and *db/transaction.db* files. Remove all the files in your back end store directory (*db/hashnode* by default). Then change your configuration file to use the RocksDB back end and restart.

To import your old database, start by shutting the server down. Then modify the configuration file by renaming your *\[node\_db\]* stanza to *\[import\_db\]*. Create a new *\[node\_db\]* stanza and specify a RocksDB back end with a different directory. Start the server with the command **Suhibd --import**. When the import finishes gracefully stop the server (\`Suhibd stop\`). Please wait for Suhibd to stop on its own because it can take several minutes for it to shut down after an import. Remove the old database, put the new database into place, remove the *\[import\_db\]* section, change the *\[node\_db\]* section to refer to the final location, and restart the server.
...
You need to configure the [NodeBackEnd](https://wiki.Suhib.com/NodeBackEnd) that you want the server to use. See above for an example RocksDB configuration.
...
Suhibd version 0.20.1 has been released. This release is currently the tip of the [develop](https://github.com/Suhib/Suhibd/tree/develop) branch and the tag is [0.20.1](https://github.com/Suhib/Suhibd/tree/0.20.1).
...
-   Suhibd will crash randomly.
    -   Entries in the three parts of the order book are missing or do not match. In such a case, Suhibd will crash.
-   Server loses sync randomly.
    -   This is due to Suhibd restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   Server goes 'offline' randomly.
    -   This is due to Suhibd restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   **complete\_ledgers** part of **server\_info** output says "None".
    -   This is due to Suhibd restarting and reconstructing the ledger after it crashes.
    -   If the node back end is corrupted or has been moved without being renamed in Suhibd.cfg, this can cause Suhibd to crash and restart.

**Toolchain support**

Starting with this release, the minimum supported version of GCC used to compile Suhibd is v4.8.
...
-   Reset liquidity before retrying SuhibCalc.
...
-   Added Debian-style initscript (doc/Suhibd.init).
-   Updated default config file (doc/Suhibd-example.cfg) to reflect best practices.
...
Suhibd version 0.19 has now been released. This release is currently the tip of the [release](https://github.com/Suhib/Suhibd/tree/release) branch and the tag is [0.19.0](https://github.com/Suhib/Suhibd/tree/0.19.0).
...
Suhibd version 0.16 has now been released. This release is currently the tip of the [master](https://github.com/Suhib/Suhibd/tree/master) branch and the tag is [v0.16.0](https://github.com/Suhib/Suhibd/tree/v0.16.0).
...
-   Suhibd no longer throws and exception on exiting
-   Better error reporting
-   Suhib-lib tests have been ported to use the Mocha testing framework

**Validators**

Suhib Labs is now running five validators. You can use this template for your `validators.txt` file:
...
You need to configure the [NodeBackEnd](https://wiki.Suhib.com/NodeBackEnd) that you want the server to use. In most cases, that will mean adding this to your configuration file:
...
Suhibd version 0.14 has now been released. This release is currently the tip of the [master](https://github.com/Suhib/Suhibd/tree/master) branch and the tag is [v0.12.0](https://github.com/Suhib/Suhibd/tree/v0.14.0).
...
Suhibd version 0.12 has now been released. This release is currently the tip of the [master branch](https://github.com/Suhib/Suhibd/tree/master) and can be found on GitHub. The tag is [v0.12.0](https://github.com/Suhib/Suhibd/tree/v0.12.0).
...
This issue was caused by LevelDB periodically compacting its internal data structure. While compacting, Suhibd's processing would stall causing the node to lose sync with the rest of the network. This issue was solved by switching from LevelDB to HyperLevelDB. Suhibd operators will need to change their Suhib.cfg file. See below for configuration details.
...
Occasionally, some Suhibd servers would fail to fetch all ledgers. This left gaps in the local history and caused some API calls to report incomplete results. The ledger fetch code was rewritten to both prevent this and to repair any existing gaps.
...
**Update April 2014** - Due to a vulnerability in OpenSSL the validator keys above have been cycled out, the five validators by SuhibLabs use the following keys now:
